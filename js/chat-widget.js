(() => {
  ...
  const AGENT_NAME      = _cfg.name          || 'Alex';
  const SUBTITLE        = _cfg.subtitle      || 'Get a free quote in minutes';
  const COLOR_PRIMARY   = _cfg.colorPrimary  || '#8000FF';
  const COLOR_DARK      = _cfg.colorDark     || '#6600CC';
  const COLOR_HEADER    = _cfg.colorHeader   || '#060F22';
  ...
  style.textContent = `
    ...
    #wg-trigger {
      ...
      box-shadow: 0 4px 16px rgba(128,0,255,0.45);
    ...
    #wg-trigger:hover {
      ...
      box-shadow: 0 8px 28px rgba(128,0,255,0.6);
    ...
    .wg-call-bar:hover { background: rgba(26,0,48,0.95); }
    ...
    .wg-send:hover:not(:disabled) {
      ...
      box-shadow: 0 4px 14px rgba(128,0,255,0.5);
    }
  ...
  function addIntroCard() {
    ...
    trust.innerHTML = `<span class="wg-intro-pill">Licensed &amp; Insured</span><span class="wg-intro-pill">Same-Day Available</span><span class="wg-intro-pill">North GA Local</span>`;
  ...
  if (AUTO_OPEN_DELAY !== null && !sessionStorage.getItem('wg_opened')) {
    setTimeout(() => {
      if (window.innerWidth > 768) {
        sessionStorage.setItem('wg_opened', '1');
        openPanel();
      }
    }, AUTO_OPEN_DELAY);
  }
